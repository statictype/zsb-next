...
| ✅ **Stale hosted schema manifest** | **Resolved 2026-06-05** via `pnpm exec sanity schema deploy`. There is a *single* deployed schema document — `_.schemas.default` — and per the [Schema Deployment docs](https://www.sanity.io/docs/apis-and-sdks/schema-deployment) it is the one artifact every Sanity app/API/Agent-Action/MCP-content-tool consumes (NOT the embedded Vercel `/studio`, which reads schema from source and was always current). A live GROQ check confirmed the redeployed doc now contains `faqItem` + `ogImage` + `metaDescription` and no longer the old Media-Library `globalDocumentReference` image shape — so `patch_document_from_json` & co. now validate against current fields. **Note:** the MCP `get_schema` / `list_workspace_schemas` tools may still report the *old* shape — that's a stale read cached in the MCP server, not a second store and not a real out-of-date manifest. It clears on a fresh MCP connection; a full `sanity deploy` (Studio app) is **not** needed and would not touch that cache. (Correcting an earlier hypothesis: there are not "two schema stores" — just one doc plus a cosmetic MCP cache.) |
