...
> **Status: superseded (2026-07-07).** `Lightbox` is now a fully controlled
> component — callers hold `useState<number | null>` (the open image's index,
> `null` = closed) and render `<Lightbox images index onClose onIndexChange />`.
> The wrap-around/keyboard/swipe math this ADR feared re-scattering stays
> *inside* the component (nav emits `onIndexChange`), so the caller boilerplate
> is two lines of state, not the old five-prop fan-out.
