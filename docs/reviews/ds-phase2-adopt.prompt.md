# DS Phase 2 — per-surface adoption (run once per surface)

Execute the migration for **one surface** against the **frozen catalog**
(`docs/reviews/<date>-ds-catalog.md`, produced by
`ds-phase1-catalog.prompt.md` and grilled by the user — check its Open
questions section is marked resolved; if not, stop). The catalog is
authoritative and closed: every verdict, target style, gap owner,
restructure design, and dissolution row is already decided. Your job is
faithful execution, not redesign.

Model: Sonnet 5 at **high effort**, adaptive thinking. Calibration (same
as the C1–C9 sweeps): follow catalog rows literally and apply blanket
rules to every member; escalate instead of improvising anything the
catalog didn't anticipate; terse output, no narration.
...
2. **Escalate, don't improvise** — a declaration with no catalog row, a
   row that no longer matches the code, a conflict between rows: leave
   that item unchanged, list it. Never invent a verdict.
3. Scope: only this surface's member files (plus the one-time step 0 and
   ...
   job).
4. Comments: strip restatement/narration in files you touch; never add
   any (CLAUDE.md rule). Delete comments describing things this sweep
   removes.
...
1. **Type** — apply each type-census row: the declaration becomes
   ...
   surface's files.
2. **Ink** — colors die (cascade), stay (row says so), per the ink census.
...
6. **Empty-husk cleanup** — unconditional, not gated on the dissolution
...
7. **Recipe dissolution** — per the dissolution map: delete slots that
...
8. **Ride-along** — comment strip (hard rule 4).
...
catalog-sanctioned exception (art/skin rows); list any such survivor
next to its row id.
5. `pnpm format`.
