# DS enforcement — conventions become gates (Fable 5)
...
- **Gates only, no cleanup.** If a gate finds a real violation in the
  current tree, allowlist or escalate it — do not fix it here. The one
  exception: violations this pass itself would create are bugs in the
  gate, not the tree.
- **Zero false positives.** A gate that flags sanctioned code (art
  ...
  current tree passes.
- Follow the repo comment policy; gate failure messages must tell the
  author what to do instead, in one line.
