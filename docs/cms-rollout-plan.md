...
_Update 2026-06-03: substring validation (#3) and the artist reverse-reference view (#5) shipped. Still pending — all migration-bearing, deferred to one migration pass: carousel collapse (#1), `dateTape` split (#2), and the `creditText`→array half of #4 (a production check found `creditText` rows on every 2022–2025 edition, so it is **not** zero-risk after all)._
...
4. `[~]` Store credit org lists as arrays, not newline-joined strings. **Partially done:** `creditOrgList` (array of organization references) shipped. Remaining: `creditText` still uses a newline-joined `value` string ("Use newlines to separate multiple names"). **Reclassified 2026-06-03 — not zero-risk:** every 2022–2025 edition has `creditText` rows, several with newline-separated multi-name values (e.g. 2025 Photo Credit = "Sorin Nainer\nDarie Dup\nStefan Bogdan\nFlorin Aldea"). Renaming `value` → an array orphans that data, so the conversion needs a data migration and moves to the migration list below. (The runtime mapper in `editions.ts` already flattens all three credit types to a newline-joined `value`, so this is an authoring-surface change only — the renderer and `CreditEntry` type are unaffected.)
...
- `[ ]` `creditText` → array: for each `edition.credits[_type == "creditText"]`, split `value` on `\n` into a new `names` array, then drop `value`. Pairs with the schema rename in #4. Dry-run first.
