import { programmeOrder } from '@calendar/calendar-filters'
...
// the programme has a first and a last event, and a panel that loops hides that.
export function eventSteps(events: CalendarEvent[], slug: string, year: number): EventSteps {
  const order = programmeOrder(events)
