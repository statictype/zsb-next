 ...
 * (home / Footer), with a `1` fallback — the bracketed calc()s are that
 * mechanism. The `variant` axis (light | dark) only swaps the rotating ring's
 * text fill — the center heart is always the accent. The elastic hover-scale
 * lives on `body` below (a `linear()` spring), which is what lets the
 * component itself stay a server component.
 */
